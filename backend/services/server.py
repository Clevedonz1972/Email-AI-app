...
class EmailPreferences(BaseModel):
    stress_level: Optional[str] = Field(
        None, description="Default stress level for similar emails"
    )
    handling_strategy: Optional[str] = Field(
        None, description="Preferred way to handle similar emails"
    )
    reminders: Optional[Dict] = Field(None, description="Reminder settings")
    auto_categorize: Optional[bool] = Field(
        None, description="Whether to auto-categorize similar emails"
    )
...
class BatchAction(BaseModel):
    ...
    parameters: Optional[Dict] = Field(
        None, description="Additional parameters for the action"
    )
...
async def get_emails(
    ...
    sort_by: str = Query(
        "date", description="Sort criteria (date, priority, stress_level)"
    ),
    ...
    if stress_level:
        filtered_emails = [
            e for e in filtered_emails if e.get("stress_level") == stress_level
        ]
    if category:
...
async def batch_process(action: BatchAction):
    ...
    return {"success": len(errors) == 0, "processed": processed, "errors": errors}
