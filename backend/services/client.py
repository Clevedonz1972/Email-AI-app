...
class EmailData:
    ...
    category: Optional[str] = (
        None  # NEW: Email category (e.g., "Bills", "Social", "Work")
    )
...
class EmailAPIClient:
    ...
    def _handle_request_error(
        self, e: requests.RequestException, operation: str
    ) -> Dict:
    ...
    def mark_as_read(self, email_id: str) -> bool:
        ...
        try:
            response = self.session.patch(f"{self.base_url}/api/emails/{email_id}/read")
            response.raise_for_status()
            return True
        except requests.RequestException as e:
    ...
    def send_email(
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/emails/send", json=payload
            )
            response.raise_for_status()
            return True
        except requests.RequestException as e:
    ...
    def search_emails(self, query: str) -> List[EmailData]:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/emails/search", params={"q": query}
            )
            response.raise_for_status()
            return [EmailData(**email) for email in response.json()]
        except requests.RequestException as e:
    ...
    def update_email_preferences(self, email_id: str, preferences: Dict) -> Dict:
        """
        ...
        try:
            response = self.session.patch(
                f"{self.base_url}/api/emails/{email_id}/preferences", json=preferences
            )
            response.raise_for_status()
    ...
    def get_email_analytics(self, user_id: str) -> Dict:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/analytics/emails", params={"user_id": user_id}
            )
            response.raise_for_status()
    ...
    def batch_process_emails(
        self, email_ids: List[str], action: str, parameters: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/emails/batch", json=payload
            )
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
