...
class AIResponse(BaseModel):
    ...
    def parse_ai_response(cls, response: str) -> "AIResponse":
        try:
            ...
            return cls(stress_level=StressLevel.MEDIUM, priority=Priority.MEDIUM)
...
class AIHandler:
    ...
    async def generate_reply(
        self, content: str, tone: Optional[str] = "professional"
    ) -> ReplyResponse:
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"Generate a {tone} reply and return JSON with fields: content (str), tone (str), formality_level (int 1-5)"
                    },
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Analyze this email and return JSON with fields: summary, stress_level (LOW/MEDIUM/HIGH), priority (LOW/MEDIUM/HIGH), action_items (list), sentiment_score (float between -1 and 1)"
                    },
                    {"role": "user", "content": content},
                ],
            )
            result = json.loads(response.choices[0].message.content)
            # Convert enum fields to uppercase to match the expectation
            if "stress_level" in result and isinstance(result["stress_level"], str):
                result["stress_level"] = result["stress_level"].upper()
            ...
            logger.error(f"Email analysis validation failed: {str(e)}")
            raise HTTPException(status_code=422, detail=str(e))
        except json.JSONDecodeError as e:
            logger.error(f"Email analysis JSON parsing failed: {str(e)}")
    ...
    async def generate_response_suggestion(
        self, email_content: str, user_preferences: Dict
    ) -> str:
        """Generate a response suggestion based on the email content and user preferences."""
        try:
            if self.testing:
                return "Test response suggestion"

            assistance_level = user_preferences.get("ai_assistance", {}).get(
                "level", "balanced"
            )
...
"""
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an AI assistant helping to draft email responses with clarity and consideration for neurodiversity.",
                    },
    ...
    async def simplify_content(self, content: str) -> str:
        ...
        try:
            if self.testing:
                return "Simplified test content"
    ...
    def _parse_json_response(self, json_str: str, model: BaseModel) -> Dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"AI response missing required fields: {str(e)}"
            )
